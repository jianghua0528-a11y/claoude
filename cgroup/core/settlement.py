...
口径区分 (关键, 工资口径 ≠ 闭环口径):
  ...
  · 实操口径 (settlement): 公司实际付/收 —— 直结单妈咪, 不进公司账
"""
from dataclasses import dataclass
from typing import Optional

# 分成档 (每单直接定, 不再绑妈咪状态): 艺人 / 妈咪 / 公司
RATIOS = {
    "标准":   dict(artist=0.70, mama=0.20, company=0.10),  # 有妈咪默认
    "直结":   dict(artist=0.70, mama=0.30, company=0.00),  # 妈咪直结艺人, 公司不沾
    "自单":   dict(artist=0.90, mama=0.00, company=0.10),  # 无妈咪
    "全归艺人": dict(artist=1.00, mama=0.00, company=0.00),  # 特批不抽: 公司+妈咪都不抽
}
...
class Order:
    ...
    M: float = 0.0               # 现金 (MYR)
    O: float = 0.0               # 门票 (MYR)
    mode: str = "标准"           # 分成档: 标准/直结/自单/全归艺人 (每单直接定)
...
def compute(o: Order) -> Result:
    ...
    # 标准: 挂账单 → 公司向妈咪应收; 现金单 → 公司付妈咪反水
    if K > 0:    # 挂账(含混合单的挂账部分)
        res.mama_receivable = K * (1 - r["mama"]) + O   # 标准 = K×0.8 + O
